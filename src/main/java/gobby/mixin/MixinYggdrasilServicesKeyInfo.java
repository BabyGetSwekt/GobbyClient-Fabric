...
public class MixinYggdrasilServicesKeyInfo {
    ...
    private byte[] gobby$replaceKnownWrongBase64(Base64.Decoder decoder, String signature, Operation<byte[]> decode) {
        try {
            ...
            try {
                ...
                //if (Utils.isOnSkyblock()) {
                    ...
                    if (!ERRONEUS_SIGNATURE_HASHES.contains(signatureHashCode)) {
                        ...
                        LOGGER.warn("[Gobbyyyy Base64 Fixer] Failed to decode base64 string No.{}: {}", ERRONEUS_SIGNATURE_HASHES.size() - 1, signature);
                    } else {
                        LOGGER.warn("[Gobbyyyy Base64 Fixer] Failed to decode the base64 string No.{} again", ERRONEUS_SIGNATURE_HASHES.indexOf(signatureHashCode));
                    }
                //}
            }
            throw e;
